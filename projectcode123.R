...
write.csv(film, file = "Film_data.csv", row.names = FALSE)
...
write.csv(film, "Film_Data2.csv", row.names = FALSE)

#-------------------------------------------------------
film <- read.csv("Film_Data2")
...
lm.full <- lm(gross_earning_after~theatre_count+IMDB_Rating+Tomato_Meter+Tomato_Rating+
                Tomato_User_Meter+Tomato_User_Rating+Action+Adventure+Adult+Animation+Biography+
...
lm2 <- lm(gross_earning_after~theatre_count+Tomato_Rating+Tomato_User_Meter+Tomato_User_Rating+Action
          +Adventure+Animation+Comedy+Drama+Family+Fantasy+Horror+Music+Short+Sport+Thriller
          +Western,data=film.train)
...
#------------------------------
# Lasso Regularization, 3rd model
cc <- complete.cases(film.train)
cv1 <- cv.glmnet(data.matrix(film.train[cc,-c(1,2,3,4,5,8,13,37,38)]), film.train[cc,38], alpha=1)
cv1$lambda.1se
cv1$lambda

model <- glmnet(data.matrix(film.train[cc,-c(1,2,3,4,5,8,13,37,38)]), film.train[cc,38], alpha=1)
coef(model)[,17]

lm3 <- lm(gross_earning_after~ theatre_count+ Tomato_Rating + Tomato_User_Meter + Adventure + 
            Fantasy, data = film.train)
...
lm4 <- lm(formula = gross_earning_after ~ theatre_count + Tomato_Rating + 
     Tomato_User_Meter + Tomato_User_Rating + Action + Adventure + 
     Animation + Comedy + Drama + Family + Fantasy + Horror + 
     Music + Sport + Western, data = film.train)
summary(lm4)
...
r_student1 <- rstudent(lm3)
qqnorm(r_student1, xlab = "external standardized residuals",
       ylab = "Probability", main = "Normal Probablity plot")
qqline(r_student1)
# the plot is positively skewed, presence of outliers 

# External studentized residuals vs predicted response
pred_data <- predict(lm3, newdata = film.train)
plot(pred_data[1:920], r_student1,
     main = " The residual plot between r-student residual and predicted y values",
...
Cook_dis <- cooks.distance(lm3)
# we get the list of value for each observation, the influential observation will the 
# one's where the cook's value is >1
(1:length(Cook_dis))[Cook_dis>1]
# there is no influential point

# DFFITS, we get one value per observation
diffits01 <- dffits(lm3)
diffits01

# If dffits_i > 2*sqrt(p/n), then consider observation i as influential
(1:length(diffits01))[diffits01 > 1 | diffits01< -1] 
#
diffits01[525]

# removing the outlier
film.train <- film.train[-match(939,as.list(rownames(film.train))),]

#------------------
# Lasso Regularization, 5th model
cc <- complete.cases(film.train)
cv1 <- cv.glmnet(data.matrix(film.train[cc,-c(1,2,3,4,5,8,13,37,38)]), film.train[cc,38], alpha=1)
cv1$lambda.1se
cv1$lambda
...
lm4 <- lm(gross_earning_after~ theatre_count+ Tomato_Rating + Tomato_User_Meter + Adventure + 
         Fantasy, data = film.train)
...
bc01 <- boxcox(lm3,plotit=T, interp=T)
bc01   # Lists log-likelihoods and corresponding lambdas.

maxlambda01 <- bc01$x[bc01$y==max(bc01$y)]
maxlambda01 #[1] -0.2626

# 5th Model
#lm5 <- lm(formula = ((gross_earning_after)^(-0.2606)) ~ theatre_count + Tomato_Rating + Tomato_User_Meter + 
#            Adventure + Animation + Crime + Drama + Family + Fantasy + 
#            Horror + Music + Short + Documentary, data = film.train)
lm5 <- lm(((gross_earning_after)^(-0.2606)) ~theatre_count+ Tomato_Rating + Tomato_User_Meter + Adventure + 
            Fantasy, data = film.train)
summary(lm5) # .5203

pred5 <- predict(lm5, newdata =  film.test)
mse5 <- mean((film.test$gross_earning_after-(pred5)^(1/(-0.2606)))^2, na.rm = TRUE)
...
# External studentized residuals vs predicted response
pred_data <- predict(lm5, newdata = film.train)
plot(pred_data[1:919], r_student2,
     main = " Plot between r-student residual and predicted y values",
     xlab = "predicted y values", ylab = " Residuals")

#------------------------------------
# 6th Model
lm6 <- lm(formula = (log(gross_earning_after)) ~theatre_count+ Tomato_Rating + Tomato_User_Meter + Adventure + 
            Fantasy, data = film.train)
summary(lm6) # .5032
...
# External studentized residuals vs predicted response
pred_data <- predict(lm6, newdata = film.train)
plot(pred_data[1:919], r_student3,
     main = " Plot between r-student residual and predicted y values",
     xlab = "predicted y values", ylab = " Residuals")
...
film1<-film[sample(nrow(film)),]

#Create 10 equally size folds
folds <- cut(seq(1,nrow(film1)),breaks=5,labels=FALSE)
...
for(i in 1:5){
  ...
  testData <- film1[testIndexes, ]
  trainData <- film1[-testIndexes, ]
  lm6 <- lm(formula = (log(gross_earning_after)) ~theatre_count+ Tomato_Rating + Tomato_User_Meter + Adventure + 
              Fantasy, data = trainData)
  pred6 <- predict(lm6, newdata = testData)
  mse_ <- mean((testData$gross_earning_after-exp(pred6))^2, na.rm = TRUE)
  mse_full[i] <- mse_
}
sum(mse_full)/5
#--------------------------------------------------------------------------------------------------
